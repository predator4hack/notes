...
## What goes into `__pycache__`

`__pycache__` stores **compiled Python bytecode files** (`.pyc` files).
...
1. **Python bytecode** (instructions for the Python Virtual Machine)
    
2. **Metadata**
    
    - Python version
        
    - timestamp / hash of the source file

The bytecode looks like low-level instructions such as:
```
LOAD_CONST  
STORE_NAME  
CALL_FUNCTION  
...
These are executed by the **Python interpreter’s virtual machine**.
You can actually see this using:
import dis  
...
That is the **bytecode stored in `.pyc` files**.

## Why Python creates `__pycache__`

To **avoid recompiling `.py` files every time**.

Without cache:

```
.py → compile → bytecode → run
```
...
```
.py → check .pyc  
      if valid → run bytecode directly
```
This makes imports faster, especially in large projects.

## Why the filename looks like this
